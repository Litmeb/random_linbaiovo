# 什么随机生成 / random_linbaiovo

一个「猜发帖人」小游戏：从 Bangumi 小组的帖子中随机抽一条，显示标题和正文，让你在两名作者里猜是谁发的。赛博风界面，带积分、排行榜与历史统计。

## 功能概览

- **出题**：从 `posts.json` 随机取一条帖子，展示标题 + 正文；两个选项为一正一扰（正确答案 + 常发帖作者表中的随机干扰项）。
- **计分**：答题过程中每秒 -1 分；答对 +7，答错 -10；支持暂停、重开，可清除「历史最高」记录（不影响已上传排行榜与 delete token）。
...
1. 用本地 HTTP 服务打开（否则无法加载 `posts.json` 和 Firebase），例如：
   ```bash
...
```bash
python bgm_group_crawler.py https://bgm.tv/group/psycho --pages 1 --output posts.json
```
