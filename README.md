...
**本分支为茶话会特供**。一个「猜发帖人」小游戏：从茶话会的帖子中随机抽一条，显示标题和正文，让你在两个选项中猜谁是楼主。赛博风界面，带积分、排行榜与历史统计。
...
- **楼主统计总览**：按楼主展示答对/答错次数与正确率。只显示答对+答错≥3的楼主，按正确率排序，正确率相同则答得多的优先；标题旁问号可查看规则。本地自动记录每位楼主的作答情况，点击「上传我的数据」可把本地数据累加到云端并清除本地。
- **本地记忆**：历史最高分、上次排行榜使用的「你的名字」、楼主级答对/答错统计会保存在浏览器本地。
...
排行榜与楼主统计需配置 Firebase，详见 [FIREBASE_SETUP.md](FIREBASE_SETUP.md)。
...
- `hot_posts.json` — 题目数据（楼主、标题、正文、链接等）
- `bgm_group_crawler.py` — Bangumi 小组帖子爬虫
- `FIREBASE_SETUP.md` — Firebase Firestore 配置说明（排行榜 + 楼主统计）
